class CrudsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @crud.save
        format.html { redirect_to crud_url(@crud), notice: "Crud was successfully created." }
  ...
  def update
    respond_to do |format|
      if @crud.update(crud_params)
        format.html { redirect_to crud_url(@crud), notice: "Crud was successfully updated." }
  ...
  private
    ...
    def crud_params
      params.require(:crud).permit(:name, :age)
    end
end
